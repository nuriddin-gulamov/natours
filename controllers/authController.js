...
const createAndSendToken = (user, statusCode = 200, res) => {
  const token = signToken(user._id);

  // SEND TOKEN FROM COOKIE //
  const cookieOptions = {
    expires: new Date(
...
exports.isLoggedIn = catchAsync(async (req, res, next) => {
  if (req.cookies.jwt) {
    ...
    // THERE IS A LOGGED IN USER IF THE CODE COMES UP TO THIS POINT //
    res.locals.user = currentUser;
    return next();
  }
  next();
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        'There was an error sending the email. Pleasy try again later.',
        500
      )
    );
  }
